...
class Display
{
    ...
    static public var samples
        = Data.SAMPLE.split("\n").map(fromString);
    
    public var signals:Array<Signal>;
    public var digits:Array<Signal>;
    public function new(signals:Array<Signal>, digits:Array<Signal>)
    {
        this.signals = signals.map(s->s.sort());
        this.signals.sort(sortSignals);
        this.digits = digits.map(s->s.sort());
    }
    
    static function sortSignals(a:Signal, b:Signal) return a.length - b.length;
    
    static public function fromString(data:String)
    {
        var splitData = data.split(" | ");
        return from2Strings(splitData[0], splitData[1]);
    }
    
    static public function from2Strings(signals:String, digits:String)
    {
        return new Display(signals.split(" "), digits.split(" "));
    }
}

@:forward
abstract Signal(String) from String
{
    ...
    public function has(segment:Segment)
    {
        return this.indexOf(segment) != -1;
    }
    
    @:op(A - B)
    public function sans(shorter:Signal):Signal
    {
        var result:Signal = this;
        for (i in 0...shorter.length)
            result = result.without(shorter[i]);
        return result;
    }
    
    @:op(A - B)
    public function without(segment:Segment):Signal
    {
        return this.split(segment).join("");
    }
    
    public function sansLone(shorter:Signal):Segment
    {
        var result = sans(shorter);
        if (result.length > 1)
            throw 'Expected 1 missing long:$this short:$shorter';
        return cast result;
    }
    
    @:arrayAccess
    inline function get(i:Int):Segment return this.charAt(i);
}

enum abstract Segment(String) from String to String
{
    var A = "a";
    var B = "b";
    var C = "c";
...
class Data
{
    ...
    inline static public var SAMPLE =
